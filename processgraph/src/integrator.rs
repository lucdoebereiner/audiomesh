pub fn runge_kutta_4<A>(f: &dyn Fn(&[f64], &A) -> Vec<f64>, state: &[f64], additional_vars: &A, h: f64) -> Vec<f64> {
...
pub fn runge_kutta_5<A>(f: &dyn Fn(&[f64], &A) -> Vec<f64>, state: &[f64], additional_vars: &A, h: f64) -> Vec<f64> {
    let k1 = f(state, additional_vars);
    let k2_state : Vec<f64> = state
        .iter()
        .enumerate()
    ...
    let k3_state : Vec<f64> = state
        ...
        .enumerate()
        .map(|(index, var)| {
            var + ((h * 1.0/8.0) * k1[index]) + ((h * 1.0/8.0) * k2[index])
        })
        .collect();
    let k3 = f(&k3_state, additional_vars);
    let k4_state : Vec<f64> = state
        ...
        .enumerate()
        .map(|(index, var)| {
            var - (h * (1.0/2.0) * k2[index]) + (h  * k3[index])
        })
        .collect();
    let k4 = f(&k4_state, additional_vars);
    let k5_state : Vec<f64> = state
        ...
        .enumerate()
        .map(|(index, var)| {
            var + (h * (3.0/16.0) * k1[index]) + (h * (9.0/16.0) * k4[index]) 
        })
        .collect();
    ...
    let k6_state : Vec<f64> = state
        ...
        .enumerate()
        .map(|(index, var)| {
            var - (h * (3.0/7.0) * k1[index]) + (h * (2.0/7.0) * k2[index]) + (h * (12.0/7.0) * k3[index]) - (h * (12.0/7.0) * k4[index]) + (h * (8.0/7.0) * k5[index])
        })
        .collect();
    ...
    let result : Vec<f64> = state
        ...
        .enumerate()
        .map(|(index, var)| {
            var + (((7.0 * k1[index]) + (32.0 * k3[index]) + (12.0 * k4[index]) + (32.0 * k5[index]) + (7.0 * k6[index])) * (h/90.0))
        })
        .collect();
    result
